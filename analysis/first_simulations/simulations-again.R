...
setwd("/mnt/expressions/benjamin_vernot/faunal_mismapping/data/once-again/5_parsed-bams")
...
for (directory in directories) {
  ...
  for (inner_directory in inner_directories) {
    combined_direc <- paste(directory, "/", inner_directory, sep="")
    inner_file <- list.files(combined_direc)
    
    combined_file <- paste(combined_direc, "/", inner_file, sep="")
